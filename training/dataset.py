...
class FelixLRS2Dataset(Dataset):
    ...
    def __init__(self, alignment_file, root_dir, frames_length = 300, tokens_length = 10000, transform=None):
        """
        Args:
    ...
    def add_padding_to_video(self, video_array):
        ...
        if num_frames > self.frames_length:
            ...
            padded_video_array = downsampled_video_array[:self.frames_length]
        else:
    ...
    def load_data(self, path):
        ...
        result['target_lengths'] = len(alignments) # For CTC Loss
        ...
        # Add padding
        result['frames'] = self.add_padding_to_video(frames)
        # result['alignments'] = alignments

        result['alignments'] = self.add_padding_to_tokens(alignments)
    ...
    def __getitem__(self, idx):
        ...
        result = self.load_data(clip_path)
        
        if self.transform:
            result['frames'] = self.transform(result['frames'])
